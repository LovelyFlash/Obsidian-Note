...
### 3.光程 光程差
介质折射率
$$\huge n=\dfrac{c}{v}=\sqrt{\varepsilon_r \mu_r}$$
介质中的波长
$$\huge \lambda=vT=\dfrac{cT}{n}=\dfrac{\lambda_0}{n}$$
定义光波在介质中经历的几何路程$d_i$与介质折射率$n_i$的乘积为光程$L$
$$\huge L=\sum_{i}n_id_i$$
此时相位差可统一为
$$\huge \Delta\varphi=\dfrac{2\pi}{\lambda_0}\delta$$
>$\delta$ 光程差
...
### 1.光振动
振动方程
$$\huge E=E_0\cos(\omega t+\varphi)$$
波动方程
$$\huge E=E_0\cos\left(\omega\left(t-\dfrac{x}{v}\right)+\varphi\right)$$
光强
$$\huge I\propto E_0^2$$
### 2.干涉条件（相干波）
- 振动方向相同
...
$$\huge I=I_1+I_2+2\sqrt{I_1I_2}\cdot\overline{\cos\Delta\varphi}$$
其中$\overline{\cos\Delta\varphi}$为干涉项，且
$$\huge \Delta\varphi=\varphi_{20}-\varphi_{10}-\dfrac{2\pi}{\lambda}(r_2-r_1)$$
#### 非相干叠加
由于不同频率的光波列均匀分布，故有
$$\huge \overline{\cos\Delta\varphi=0},\quad I=I_1+I_2$$
#### 相干叠加
此时$\overline{\cos\Delta\varphi}=\cos\Delta\varphi$，若两光波强度相等，则$$\huge I=4I_1\cos^2\dfrac{\Delta\varphi}{2}$$
干涉极大极小条件为
$$\huge\Delta\varphi=\begin{cases}
\pm2k\pi,\quad I_{max}=I_1+I_2+2\sqrt{I_1I_2},\quad 明纹\\
\pm(2k+1)\pi,\quad I_{min}=I_1+I_2-2\sqrt{I_1I_2},\quad 暗纹
\end{cases}$$
#### 衬比度
衬比度定义为
$$\huge V=\dfrac{I_{max}-I_{min}}{I_{max}+I_{min}}=\dfrac{2\sqrt{I_1I_2}}{I_1+I_2}$$
...
![[杨氏双缝干涉装置图.svg]]
#### 光程差条件
$$\huge \delta=r_2-r_1=d\sin\theta=\begin{cases}
k\lambda,\quad max\\
...
设$P$点坐标$x$（距0点很近），有
$$\huge \delta=d\sin\theta=d\dfrac{x}{D}$$
即
$$\huge x=\begin{cases}
...
#### 强度分布
$$\huge I_\theta=4I_0\cos^2\dfrac{\Delta\varphi}{2}=4I_0\cos^2(\dfrac{\pi d\sin\theta}{\lambda})$$
>$$\huge\Delta\varphi=\varphi_2-\varphi_1-\dfrac{2\pi}{\lambda}(r_2-r_1)=-\dfrac{2\pi}{\lambda}d\sin\theta$$
...
$$\huge \delta=2d+\dfrac{\lambda}{2}=\begin{cases}
...
衍射现象实为无穷多无限小子波的干涉效应
菲涅耳衍射公式：
$$\huge y=\int_\Sigma\d y=\int_\Sigma C\dfrac{f(\alpha)a(\Sigma_i)}{r}\cos[2\pi(\dfrac{t}{T}-\dfrac{r}{\lambda})]\d\Sigma$$
...
$$\huge a\sin\theta=\begin{cases}
(2k+1)\dfrac{\lambda}{2},\quad max\\
k\lambda,\quad min\\
0,\quad中央主极大
\end{cases}\quad k=\pm1,\pm2\cdots$$
#### 强度分布
$$\huge I_\theta=I_0\left(\dfrac{\sin\alpha}{\alpha}\right)^2,\alpha=\dfrac{\pi a\sin\theta}{\lambda}$$
#### 条纹宽度
中央主极大宽度：$$\huge \Delta x_0=2f\dfrac{\lambda}{a}$$
次极大宽度：
$$\huge \Delta x=\dfrac{f\lambda}{a}=\dfrac{1}{2}\Delta x_0$$
次极大条纹宽度为中央主极大的**一半**
#### 动态反应
- 波长越长，条纹越宽
- 缝宽越小，条纹越宽
